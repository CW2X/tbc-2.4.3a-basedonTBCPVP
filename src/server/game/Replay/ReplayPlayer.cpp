    ...
    while (true)
    {
        ...
        else
        {
            ...
            while (true)
            {
                ...
                {
                    if (_player)
                        ChatHandler(_player->GetSession()).PSendSysMessage("[Replay] Invalid packet size [opcode %s|size %u|time %u]", GetOpcodeNameForLogging(static_cast<OpcodeClient>(opcode)).c_str(), size, nextTime);
                    return false;
                }
                if (readValue == 256)
                    break;
                ...
                {
                    if (_player)
                        ChatHandler(_player->GetSession()).PSendSysMessage("[Replay] Invalid packet (truncated) [opcode %s|size %u|time %u]", GetOpcodeNameForLogging(static_cast<OpcodeClient>(opcode)).c_str(), size, nextTime);
                    return false;
                }
            }
